...
export default function Create({ warehouses }) {
    const { data, setData, post, processing, errors } = useForm({ name:'',email:'',password:'',password_confirmation:'',role:'viewer',warehouse_id:'',is_active:true });
    return (
        <AuthenticatedLayout title="Tambah User"><Head title="Tambah User" />
            ...
            <FormCard onSubmit={e=>{e.preventDefault();post('/users');}} processing={processing}>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <FormField label="Role *" error={errors.role}><Select value={data.role} onChange={e=>setData('role',e.target.value)} options={[{value:'admin',label:'Admin'},{value:'manager',label:'Manager'},{value:'staff',label:'Staff'},{value:'viewer',label:'Viewer'}]}/></FormField>
