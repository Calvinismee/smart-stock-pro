...
export default function Create({ products, sourceWarehouses, destinationWarehouses }) {
    const { data, setData, post, processing, errors } = useForm({ product_id:'',source_warehouse_id:sourceWarehouses.length===1?sourceWarehouses[0].id:'',destination_warehouse_id:'',quantity:'',transfer_date:new Date().toISOString().split('T')[0],notes:'' });
    return (
        <AuthenticatedLayout title="Buat Transfer"><Head title="Buat Transfer" />
            ...
            <FormCard onSubmit={e=>{e.preventDefault();post('/stock-transfers');}} processing={processing} submitLabel="Proses Transfer">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <FormField label="Gudang Asal *" error={errors.source_warehouse_id}><Select disabled={sourceWarehouses.length===1} value={data.source_warehouse_id} onChange={e=>setData('source_warehouse_id',e.target.value)} options={sourceWarehouses.map(w=>({value:w.id,label:w.name}))} placeholder="Pilih gudang asal"/></FormField>
                    <FormField label="Gudang Tujuan *" error={errors.destination_warehouse_id}><Select value={data.destination_warehouse_id} onChange={e=>setData('destination_warehouse_id',e.target.value)} options={destinationWarehouses.filter(w=>w.id!=data.source_warehouse_id).map(w=>({value:w.id,label:w.name}))} placeholder="Pilih gudang tujuan"/></FormField>
