...
export default function CreateIn({ products, warehouses, suppliers }) {
    const { data, setData, post, processing, errors } = useForm({ product_id:'',warehouse_id:warehouses.length===1?warehouses[0].id:'',supplier_id:'',quantity:'',transaction_date:new Date().toISOString().split('T')[0],notes:'' });
    return (
        <AuthenticatedLayout title="Barang Masuk"><Head title="Barang Masuk" />
            ...
            <FormCard onSubmit={e=>{e.preventDefault();post('/stock-transactions/store-in');}} processing={processing} submitLabel="Simpan Barang Masuk">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <FormField label="Gudang *" error={errors.warehouse_id}><Select disabled={warehouses.length===1} value={data.warehouse_id} onChange={e=>setData('warehouse_id',e.target.value)} options={warehouses.map(w=>({value:w.id,label:w.name}))} placeholder="Pilih gudang"/></FormField>
